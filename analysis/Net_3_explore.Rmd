...
Net1 UMI/fastq:  0.03347032 0.02509783 0.02486150 0.01652832 
Mayer UMI/fastq: 0.01647986  
Net3 UMI/fastq: 0.042116650 0.024026818 0.007985786 0.048030775 0.009113515 0.051427323 0.043522196 0.030275253
...
Problem could be in the or statement from awk. It looks like we see 0s in the dedup but not in the sort. We wouldnt expect that.  


Get the 3 and 5 splice sites from the GTF file 

```{bash, eval=FALSE}
less gencode.v19.annotation.gtf | tr "\"" "\t" | awk '$3 == "exon"' | awk 'NR >=6 {if($7 == "+") print($1 "\t" $4 -1 "\t" $4 "\t" $10 "\t0\t" $7 ); else print($1 "\t" $5 -1 "\t" $5 "\t" $10 "\t0\t" $7 )}'  | sort -k1,1 -k2,2n > gencode.v19.5prime.gtf
