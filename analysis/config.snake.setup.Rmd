...
rule unzip:
    input:
        fastqc_dir + "{read_num}_fastqc.zip"
    output:
        fastqc_dir + "{read_num}_fastqc.zip/fastqc_data.txt"
    params:
        outdir = fastqc_dir
    shell:
...
rule umi_create_whitelist:
    input:
        fastq_dir + "fastq_dir + "SRR1575922_CUT.fastq",
    output:
        output + "whitelist.txt"
    params:
        bc = "NNNNNN"
    shell:
        "umi_tools whitelist --stdin {input} --bc-pattern={params.bc} --extract-method=string --log2stderr > {output}"
...
```
Just try to create whitelist first.  
...
rule umi_extract_bc_and_umi:
    ...
    params:
        bc = barcode
    shell:
        "umi_tools extract --bc-pattern={NNNNNN} --stdin {input.r1} --stdout {output.r1_ext} --read2-in {input.r2} --read2-out={output.r2_ext}  --error-correct-cell --filter-cell-barcode --whitelist={input.wl}"
```    
...
rule all:
    input:
       fastqc_dir + "SRR1575922_CUT_fastqc.html",
